		...
		public static function rleEncodeStr(str:String):String 
		{
			...
			for (var i:int = 0; i < str.length; i++) {
				...
				if (lastChar == currChar || lastChar < 0) {
					...
					if (runlength > 1) {
						if (runlength == 2) {
							output += String.fromCharCode(lastChar);
						} else {
							output += String.fromCharCode(RLE_MARKER);
							output += String.fromCharCode(runlength + FIRST_VALID_CHAR);
						}
					}
					output += String.fromCharCode(lastChar);
					runlength = 1;
		...
		public static function rleDecodeStr(str:String):String 
		{
			...
			while (i < len) {
				...
				if (encodedChar == RLE_MARKER) { 
					...
					currChar = str.charCodeAt(++i);
					for (var r:int = 0; r < runlength; r++) output += String.fromCharCode(currChar);
				} else {
